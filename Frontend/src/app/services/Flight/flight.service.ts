...
export class FlightService {
  ...
  /**
   * Sends payment + booking payload to backend. The server will simulate payment
   * and return the saved booking document.
   */
  processPayment(paymentPayload: any) {
    const jwt_token = localStorage.getItem('token');
    return this.http.post(
      `${API_PATH}/flight/booking/pay`,
      paymentPayload,
      {
        headers: { authorization: `${TOKEN_PREFIX} ${jwt_token}` },
      }
    );
  }
  ...
  updateBookingStatus(bookingId: string, data: any) {
    ...
    return this.http.put(
      `${API_PATH}/flight/booking/editbooking/${bookingId}`,
      { data },
      {
        headers: {
          authorization: `${TOKEN_PREFIX} ${jwt_token}`,
